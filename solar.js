...
function updateCharts() {
    ...
    Plotly.newPlot('annualChart', [{
        ...
        title: 'Energía Anual vs Tilt',
        xaxis: { title: 'Inclinación (°)' },
        yaxis: { title: 'Energía (kWh/m²/año)' },
        margin: { t: 40, l: 50, r: 20, b: 40 }
    }, { responsive: true, displayModeBar: false });
}
...
function optimize() {
    ...
    draw();
    alert(`✅ Inclinación óptima: ${bestTilt}° | POA máximo: ${maxPOA.toFixed(1)} W/m²`);
}
...
function reset() {
    ...
    if (animationId) {
        cancelAnimationFrame(animationId);
        animationId = null;
        document.getElementById('animBtn').innerHTML = '▶️ Animar Día';
    }
    draw();
}

function showHelp() {
    alert(`🎯 GUÍA DE USO DEL SIMULADOR SOLAR

🖱️ CONTROLES CON MOUSE:
• Arrastra el PANEL SOLAR para cambiar su inclinación
• Arrastra el SOL para modificar la hora del día
• Doble clic en el canvas para reiniciar valores
...
📊 INDICADORES:
• POA: Irradiancia sobre el plano inclinado
• Factor Coseno: Eficiencia geométrica (0-1)
• AOI: Ángulo de incidencia
• Energía Anual: Producción total kWh/m²/año

💡 TIPS PARA PUNO:
✓ Mejor tilt: 10-20° (cercano a latitud)
✓ Azimuth óptimo: 0° (norte)
✓ Alta radiación por altitud
✓ Temperaturas bajas mejoran eficiencia`);
}
...
console.log(`
╔═══════════════════════════════════════════════════════════╗
║     ☀️ SIMULADOR SOLAR FOTOVOLTAICO - PUNO, PERÚ ☀️      ║
╠═══════════════════════════════════════════════════════════╣
║  📍 Ubicación: Puno (-15.84°, -70.02°)                   ║
║  ⛰️  Altitud: 3,827 msnm                                  ║
║  ☀️ GHI Promedio: 6.2 kWh/m²/día                         ║
║  🌡️ Temperatura: 8-10°C                                   ║
║                                                           ║
║  🎮 CONTROLES:                                            ║
║  • Arrastra el panel o el sol con el mouse               ║
║  • Usa las flechas del teclado                           ║
║  • Presiona ESPACIO para animar                          ║
║  • Doble clic para reiniciar                             ║
║                                                           ║
║  🚀 ¡Explora cómo la geometría solar afecta la energía!  ║
╚═══════════════════════════════════════════════════════════╝`);
